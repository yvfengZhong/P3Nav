...
from robouniview.models.transformers.ops.uvformer.functions import MSDeformAttnFunction, ms_deform_attn_core_pytorch_onnx
...
class MSDeformAttn(nn.Module):
    def __init__(self, global_config, d_model=256, n_levels=4, n_heads=8, n_points=4,im2col_step=256):
        """
        ...
        self.is_onnx = getattr(global_config, 'is_onnx', False)
    ...
    def forward(self, query, reference_points, input_flatten, input_spatial_shapes, input_level_start_index,
        ...
        assert (input_spatial_shapes[:, 0] * input_spatial_shapes[:, 1]).sum() == Len_in or self.is_onnx
        ...
        if self.global_config.precision == "bf16":
            sampling_locations = sampling_locations.to(torch.bfloat16)
            attention_weights = attention_weights.to(torch.bfloat16)
        if self.is_onnx:
        ...
        output = output.to(sampling_locations.dtype)
