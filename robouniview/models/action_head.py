...
class Multi_Action_Token_FCDecoder(ActionDecoder):
    def __init__(
        ...
        if not use_diff:
            self.actions = MLPTanhHead(hidden_size, out_features)
            self.gripper = MLPNohHead(hidden_size, nclass_gripper)
        self.hidden_state = None
        self.hidden_size = hidden_size * history_len
    ...
    def forward(  # type: ignore
        ...
        mask_indices = action_mask.nonzero(as_tuple=True) # 获取掩码为True的元素及其索引，并按行列坐标返回
        action_feature = input_feature[mask_indices[0], mask_indices[1]] # 根据掩码从output_hs中选值; torch.Size([48, 2048])
        action_feature = rearrange(action_feature, "(B T) D -> B T D", B=b)

        #input_feature = input_feature[action_mask == 1]
         
        action_feature = self.mlp(action_feature) # torch.Size([4, 12, 2048]) -> torch.Size([4, 12, 1024])
        #input_feature = self.global_1d_pool(input_feature.permute(0, 2, 1)).squeeze(-1)
        # if self.use_diff:
        #     input_feature = input_feature.reshape(-1, self.window_size * input_feature.shape[1])
        #     return input_feature
