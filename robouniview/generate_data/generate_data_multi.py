...
new_path = '~/liufanfan/data/new_calvin_D_multi/'


# sys.path.append('~/yanfeng/project/robotic/RoboFlamingo/open_flamingo')
...
#dataset_path = '~/yanfeng/data/robotics/CALVIN/task_ABCD_D'  #task_D_D'
dataset_path = '~/yanfeng/data/robotics/task_D_D' 

dataset_path_env = new_path + 'env_config/'

new_calvin = new_path + 'training/'
#new_calvin = new_path + 'training_npz_pcd_new/'
#new_calvin =  '~/liufanfan/data/debug/'
...
# cam_config = {'static': {'_target_': 'calvin_env.camera.static_camera.StaticCamera', 'name': 'static', 'fov': 12, 'aspect': 1, 
#                         'nearval': 0.01, 'farval': 10, 'width': 200, 'height': 200, 
#                         'look_at': [-0.02262423511594534, -0.0302329882979393, 0.3920000493526459], 
...
def generate_npz(dataset_path_env,data,clip_files,cam_config,ii):
    ...
    # img_clip.write_gif(f'~/liufanfan/data/debug{ii}.gif', fps=30)
...
def generte_data():
    ...
    for ii,(data,clip_files,task) in enumerate(dataset):
        ...
        if ii<0:
            ...
            # 定义 look_from 和 look_at 点
            look_from = np.array(cam_config['static']['look_from'])
            look_at = np.array(cam_config['static']['look_at'])

            # 计算 forward_vector，即相机的视线方向向量
            forward_vector = look_at - look_from
            forward_vector = forward_vector / np.linalg.norm(forward_vector)  # 归一化

            # 定义全局上方向
           

            # 计算 right_vector，即相机坐标系中的x轴方向
            right_vector = np.cross(global_up, forward_vector)
            right_vector = right_vector / np.linalg.norm(right_vector)  # 归一化

            # 计算 up_vector，即相机坐标系中的y轴方向
